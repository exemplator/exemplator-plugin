...
var getFullClassNameForVariable = function (index) {
    ...
    return getPackage(declarationPos);
}
...
var getPackageName = function (importLine, typeName) {
    ...
    if (packageWithClass !== typeName)
        ...
        packageWithClass = p.substr(0, p.length - 1);
...
var sendRequest = function (exemplatorPluginView, request, page = 0) {
    ...
    xmlhttp.open("POST", "https://api.exemplator.xyz/search");
    xmlhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");

    var dict = {}

    if (request.class && request.class.package)
        dict["package"] = request.class.package.trim();
    if (request.class && request.class.name)
